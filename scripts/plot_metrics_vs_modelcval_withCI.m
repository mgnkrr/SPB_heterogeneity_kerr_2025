...
cfg = setdef(cfg, 'fonts', struct('axis',11,'label',12,'name',' Helvetica'));
...
% --- Title includes specularity threshold (optional) ---
if isfield(cfg,'title_suffix') && ~isempty(cfg.title_suffix)
    title(ax, ['Metric vs Model Median' cfg.title_suffix], 'Interpreter','none', ...
        'FontName', cfg.fonts.name, 'FontSize', cfg.fonts.label);
end
...
if any(maskN)
    draw_ci_errorbars(ax, x_ord(maskN), Y_ord(maskN), Ylo_ord(maskN), Yhi_ord(maskN), ...
        'Color', cfg.colors.non, 'CapSize', 1, 'LineW', 1.0, 'Band', true, 'Alpha', 0.25, ...
        'XSpan', xspan);
    ...
    hPub = scatter(ax, x_ord(maskN), Y_ord(maskN), 20, 'o', ...
...
if isfield(cfg,'manual_point') && isstruct(cfg.manual_point) && isfield(cfg.manual_point,'enable') && cfg.manual_point.enable
    ...
    hMan = scatter(ax, mp.x, mp.y, 20, 'o', 'MarkerFaceColor',cfg.colors.manual, ...
...
xlabel(ax, 'Mean GHF (mW m^{-2})', ...
       'Interpreter','tex','FontSize',cfg.fonts.label, ...
       'FontName', cfg.fonts.name);

ylabel(ax, metric_label(cfg.metric), ...
       'Interpreter','tex','FontSize',cfg.fonts.label, ...
       'FontName', cfg.fonts.name);
